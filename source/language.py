...
def handle_subject(data):
    ...
    if not data["subject_isPerson"]:
        ...
        if data["subject_article"] == "definite":
            if singular_1:
                sentence += get_articolo_determinativo_singular(noun) + " "
            else:
                sentence += get_articolo_determinativo_plural(noun) + " "
        ...
        if "subject_demonstrative" in data.keys():
            if data["subject_demonstrative"] == "near":
                sentence += get_pronome_vicino(noun) + " "
            elif data["subject_demonstrative"] == "far":
                sentence += get_pronome_lontano(noun) + " "

        if "subject_possessive" in data.keys():
            if data["subject_possessive"] == "io":
                sentence += get_possesive_pronoun(1, 1, noun) + " "
            elif data["subject_possessive"] == "tu":
                sentence += get_possesive_pronoun(2, 1, noun) + " "
            elif data["subject_possessive"] in ("lui", "lei"):
                sentence += get_possesive_pronoun(3, 1, noun) + " "
            elif data["subject_possessive"] == "noi":
                sentence += get_possesive_pronoun(1, 2, noun) + " "
            elif data["subject_possessive"] == "voi":
                sentence += get_possesive_pronoun(2, 2, noun) + " "
            elif data["subject_possessive"] == "loro":
                sentence += get_possesive_pronoun(3, 2, noun) + " "

        sentence += noun + " "

        if "subject_adjective" in data.keys():
            if data["subject_adjective"] is not None:
                sentence += get_adjective_in_correct_form(noun, data["subject_adjective"]) + " "

        noun_data = define_gender(noun)
        if noun_data == "m_s":
            subject_data = [3, 1, 1]
        elif noun_data == "m_p":
            subject_data = [3, 2, 1]
...
def handle_verb(data, subject_data):
    ...
    if data["tense_type"] == "present" and data['sentence_type'] == "negative" and data["mood_type"] == "imperative":
        ...
        if data["mood_type"] == "imperative":
            sentence += get_imperativo(verb, subject_data) + " "
        elif data["mood_type"] == "indicative":
            sentence += get_present_tense(verb, subject_data) + " "
        else:
            sentence += get_condizionale_simplice(verb, subject_data) + " "
    elif data["tense_type"] == "past":
        if data["mood_type"] == "indicative":
            sentence += get_past_tense(verb, subject_data) + " "
        else:
            sentence += get_condizionale_composto(verb, subject_data) + " "
    else:
        sentence += get_future_tense(verb, subject_data) + " "
    ...
    if not data["object_isPerson"]:
        ...
        if "object_adjective" in data.keys():
            if data["object_adjective"] is not None:
                sentence += get_adjective_in_correct_form(noun, data["object_adjective"]) + " "
